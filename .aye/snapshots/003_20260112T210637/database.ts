...
class Database {
  ...
  async initialize(): Promise<void> {
    ...
    this.initPromise = new Promise((resolve, reject) => {
      ...
      request.onerror = () => { logger.error('Database', 'Failed to open', request.error); reject(request.error); };
      request.onsuccess = () => { this.db = request.result; logger.info('Database', 'Initialized'); resolve(); };
  ...
  async getScanHistory(limit = 50): Promise<(ScanResult & { domain: string })[]> {
    const store = await this.getStore('scanHistory');
    const index = store.index('startTime');
    return new Promise((resolve, reject) => {
      const results: (ScanResult & { domain: string })[] = [];
